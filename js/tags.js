...
function Shape(x, y, id) {
  ...
  // But we aren't checking anything else! We could put "Lalala" for the value of x 
	...
	this.y = y || 0;
	this.w = this.basew;
	this.h = this.baseh + this.stubs*this.stubh;
	this.fill = '#AAAAAA';
...
Shape.prototype.draw = function(ctx) {
	...
	for (var i=0; i<this.stubs; i++){
		ctx.fillStyle = stub_colors[i];
		ctx.fillRect(startX, startY, stub_width, stub_height);
		startY += stub_height;
	}
}
...
Shape.prototype.transform = function(factor){
	...
	//this.baseh += (this.scale_factor * factor);
	//this.basew += (this.scale_factor * factor);;
	//this.stubh += (this.scale_factor * factor);;
	this.w += (this.scale_factor * factor);
...
function CanvasState(canvas) {
  ...
  window.addEventListener("keydown", function(e) {
	...
	//the 'a' key
	if(e.keyCode == 65 && myState.selection != null){
		console.log('adding item');
		
		myState.addShape(new Shape(20, 20, getNewID()));
	...
	//the '=' key
	if(e.keyCode == 61 && myState.selection != null){
		console.log('apply transform to increase size');
		myState.selection.transform(1);
		myState.valid = false;
	}
	
	//the '-' key
	if(e.keyCode == 173 && myState.selection != null){
		console.log('apply transform to reduce size');
		myState.selection.transform(-1);
		myState.valid = false;
	}
	
	//the 't' key
	if(e.keyCode == 84 && myState.selection != null){
		console.log('tear stub');
		console.log(myState.selection);
		if(myState.selection.stubs >=1){
			myState.selection.stubs--;
			myState.selection.h = myState.selection.baseh + myState.selection.stubs*myState.selection.stubh;
			//myState.selection.draw(canvas.getContext('2d'));
			myState.valid = false;
...
function init() {
		...
		//s.addShape(new Shape(20,20+(stub_height*6),stub_width,stub_height, '#0BB505'));
